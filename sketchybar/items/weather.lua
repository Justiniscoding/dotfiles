...
local secrets = require("helpers.secret")

local apiKey = secrets.API_KEY
local postalCode = secrets.POSTAL_CODE
...
local function updateWeather()
	local body =
		http.request(("http://api.weatherapi.com/v1/current.json?key=%s&q=%s&aqi=no"):format(apiKey, postalCode))

	if not body then
		return
	end

	local weatherInfo = json.decode(body).current

	local weatherIcon = weatherInfo.is_day and dayWeatherIcons[weatherInfo.condition.code]
		or nightWeatherIcons[weatherInfo.condition.code]

	weather:set({
		label = {
			string = math.floor(weatherInfo.temp_c) .. "°C",
		},
		icon = {
			string = weatherIcon,
		},
...
weather:subscribe("routine", updateWeather)
weather:subscribe("system_woke", updateWeather)
